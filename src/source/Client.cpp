...
runInOtherThread(false)
{
    ioBase->setMessageHandler([this](Message message, IOBase* basePtr){
        ...
        RootTrace() << "Received message from " << basePtr->sock.remote_endpoint().address().to_string() \
        << ":" << basePtr->sock.remote_endpoint().port() << " : " << ss;
...
void Client::send(const Request& request, const ResponseHandler& handler = [](const Response&){return;}) {
    Message msg;
...
void Client::send(const Request& request, ResponseHandler&& handler = [](const Response&){return;}) {
    Message msg;
    nlohmann::json serialization_request_json = request.to_json();
    msg.buildMessageFromString(serialization_request_json.dump());
    {
