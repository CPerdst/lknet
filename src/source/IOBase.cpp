...
boost::asio::ip::tcp::socket &IOBase::socket() { return sock; }
...
void IOBase::doReadHeader() {
    ...
    boost::asio::async_read(
        sock, boost::asio::buffer(headBuffer, Message::MESSAGE_HEADER_SIZE),
...
void IOBase::doReadBody() {
    ...
    boost::asio::async_read(
        sock, boost::asio::buffer(body, readMessageSize),
        [this, self](std::error_code ec, std::size_t size) {
            try {
                if (!ec) {
                    if (messageHandler) {
                    ...
                    throw std::runtime_error(
                        "socket closed as body receive failed");
                }
            } catch (std::exception &e) {
                RootError()
                    << ": closed connection with: "
                    << sock.remote_endpoint().address().to_string() << ":"
                    << sock.remote_endpoint().port() << " as: " << e.what();
                if (closeHandler) closeHandler();
            }
        });
}
...
void IOBase::doWrite() {
    ...
    int bufferSize =
        writeMessages.front().messageBodySize + Message::MESSAGE_HEADER_SIZE;
    boost::asio::async_write(
        sock, boost::asio::buffer(buffer, bufferSize),
        [this, self](std::error_code ec, std::size_t size) {
            if (!ec) {
                bool canWrite = false;
                {
                ...
                RootError() << "socket closed as for write message failed: "
                            << ec.message() << std::endl;
                if (closeHandler) closeHandler();
            }
        });
